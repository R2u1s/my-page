...
- [ ] T033 Полный прогон `quickstart.md` от начала до конца (`docker compose up`, миграция, сид, GraphQL-запрос, визуальная проверка, проверка устойчивости к недоступности backend) — финальная валидация фичи. Миграция, сид и GraphQL-запрос подтверждены (см. T005-T021, T027); визуальную проверку в браузере (`http://localhost:5173`) и проверку устойчивости к недоступности backend пользователь выполняет самостоятельно
